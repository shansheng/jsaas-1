/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     2017/4/11 星期二 下午 12:25:55                    */
/*==============================================================*/
...
(
   ...
   VIEW_ID_             VARCHAR(64) NOT NULL COMMENT '业务表单视图ID',
   ACT_DEF_ID_          CHAR(10) COMMENT 'Activiti定义ID',
   FIELD_NAME_          VARCHAR(255) NOT NULL COMMENT '字段路径',
   FIELD_LABEL_         VARCHAR(255) NOT NULL,
   EDIT_                TEXT COMMENT '编辑权限描述',
   EDIT_DF_             TEXT COMMENT '编辑权限
            格式：
            {
            all:false,
            userIds:'''',
   ...
   READ_                TEXT COMMENT '只读权限',
   READ_DF_             TEXT COMMENT '只读权限描述',
   HIDE_                TEXT COMMENT '隐藏权限',
   HIDE_DF_             TEXT COMMENT '隐藏权限描述',
   SN_                  INT COMMENT '序号',
...
ALTER TABLE BPM_FV_RIGHT COMMENT '表单视图字段权限';
...
(
   ...
   INIT_SETTING_        VARCHAR(2000) COMMENT '表单初始数据设定',
   SAVE_SETTING_        VARCHAR(2000) COMMENT '表单初始数据设定',
   PRIMARY KEY (ID_)
);
...
(
   ...
   NUMS_OF_PAGE_        INT COMMENT '每页记录数',
   ALLOW_CLOSE_         VARCHAR(20) COMMENT '是否允许关闭',
   COL_TYPE_            VARCHAR(50) COMMENT '信息栏目类型
            公告
            公司或单位新闻
            部门新闻',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
...
ALTER TABLE INS_COLUMN COMMENT '信息栏目';
...
(
   ...
   NAME_                VARCHAR(50) NOT NULL COMMENT '栏目名称',
   KEY_                 VARCHAR(50) NOT NULL COMMENT '栏目Key',
   URL_                 VARCHAR(100) NOT NULL COMMENT '栏目映射URL',
   MORE_URL_            VARCHAR(100),
   LOAD_TYPE_           VARCHAR(20) COMMENT '加载类型
            URL=URL
            TEMPLATE=模板',
   TEMP_ID_             VARCHAR(64) COMMENT '模板ID',
   TEMP_NAME_           VARCHAR(64) COMMENT '模板名称',
   ICON_CLS_            VARCHAR(20),
   MEMO_                VARCHAR(512) COMMENT '栏目分类描述',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   CREATE_BY_           VARCHAR(64) NOT NULL COMMENT '创建人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
...
ALTER TABLE INS_COL_TYPE COMMENT '栏目分类表';
...
(
   ...
   COL_WIDTHS_          VARCHAR(50) COMMENT '栏目宽
            三列格式如250,100%,400',
...
ALTER TABLE INS_PORTAL COMMENT 'PORTAL门户定义';
...
(
   ...
   COL_ID_              VARCHAR(64) NOT NULL COMMENT '栏目ID',
   WIDTH_               INT COMMENT '宽度',
   HEIGHT_              INT NOT NULL COMMENT '高度',
   WIDTH_UNIT_          VARCHAR(8) COMMENT '宽度单位
            百份比=%
            像数=px',
   HEIGHT_UNIT_         VARCHAR(8) NOT NULL COMMENT '高度单位
            百份比=%
            像数=px',
   COL_NUM_             INT COMMENT '列号',
   SN_                  INT NOT NULL COMMENT '列中序号',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
...
ALTER TABLE INS_PORT_COL COMMENT '门户栏目配置';
...
(
   ...
   USER_ID_             VARCHAR(64) COMMENT '参与人ID',
   GROUP_ID_            VARCHAR(64) COMMENT '参与组ID',
...
(
   ID_                  VARCHAR(64) NOT NULL,
   NAME_                VARCHAR(256) NOT NULL COMMENT '名称',
   ALIAS_               VARCHAR(100) COMMENT '别名',
   IS_MULTI_SELECT_     VARCHAR(64) COMMENT '是否多选',
   IS_PAGE_             VARCHAR(64) COMMENT '是否分页',
   WIN_WIDTH_           VARCHAR(64) COMMENT '打开窗??宽度',
   WIN_HEIGHT_          VARCHAR(64) COMMENT '打开窗口高度',
   CONDITION_HTML_      TEXT COMMENT '条件HTML',
   PAGE_SIZE_           TEXT COMMENT '分页条数',
   DS_ID_               VARCHAR(64) COMMENT '数据源ID',
   DS_NAME_             VARCHAR(256) COMMENT '数据源名称',
   SQL_                 TEXT COMMENT 'SQL语句',
...
ALTER TABLE SYS_CUSTOM_QUERY COMMENT '自定义查询';
...
ALTER TABLE SYS_DEF_DIALOG COMMENT '自定义对话框';
...
(
   ...
   OP_SUBJECT_          VARCHAR(256) NOT NULL COMMENT '操作描述',
   OP_DESCP_            TEXT COMMENT '操作详述',
   OP_USID_             VARCHAR(64) NOT NULL COMMENT '操作人ID',
   EXE_CLS_METHOD_      VARCHAR(256) NOT NULL COMMENT '执行的详细类及方法',
   ERR_LOG_             TEXT NOT NULL COMMENT '出错详细日志',
...
(
   CONF_ID_             VARCHAR(64) NOT NULL COMMENT '配置ID',
   LEVEL_               VARCHAR(30) NOT NULL COMMENT '等级',
   PATH_                VARCHAR(255) NOT NULL COMMENT '包名路径',
   IS_ENABLED_          VARCHAR(20) NOT NULL COMMENT '是否开启',
   CAT_                 VARCHAR(100) NOT NULL COMMENT '日志分类',
   PRIMARY KEY (CONF_ID_)
);

ALTER TABLE SYS_LOG_CONF COMMENT '日志配置';
...
(
   ...
   PATH_                VARCHAR(256) COMMENT '路径',
   SN_                  INT COMMENT '序号',
   IS_MGR_              VARCHAR(10) NOT NULL COMMENT '是否为管理菜单，
            NO=为租户可访问的菜单 
            YES=为平台管理员可访问的菜单',
...
(
   ...
   DESCP_               VARCHAR(500) NOT NULL COMMENT '描述',
...
(
   ARTICLE_ID_          VARCHAR(64) NOT NULL COMMENT '主键',
   AUTHOR_              VARCHAR(255) COMMENT '作者',
   TITLE_               VARCHAR(255) COMMENT '标题',
   CONTENT_SOURCE_URL_  VARCHAR(255) COMMENT '原文章地址',
   CONTENT_             TEXT COMMENT '图文消息页面的内容',
   DIGEST_              VARCHAR(255) COMMENT '图文消息的描述',
   SHOW_COVER_PIC_      VARCHAR(64) COMMENT '是否显示封面',
   MATERIAL_ID_         VARCHAR(64),
   FILE_ID_             VARCHAR(64),
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
...
ALTER TABLE WX_P_ARTICLE COMMENT '非SAAS微信文章';
...
(
   CONFIG_ID_           VARCHAR(64) NOT NULL COMMENT '主键',
   INIT_ID_             VARCHAR(256) COMMENT '原始ID',
   TENANT_NAME_CN_      VARCHAR(256) COMMENT '租户名称',
   APP_TOKEN_           VARCHAR(256) COMMENT '应用凭证',
   APP_ID_              VARCHAR(256) COMMENT '应用ID',
   APP_SECRET_          VARCHAR(256) COMMENT '应用密钥',
   ACCESS_TOKEN_        VARCHAR(256) COMMENT '获取到的凭证(访问凭证)',
   EXPIRES_IN_          VARCHAR(256) COMMENT '凭证有效时间，单位：秒',
   EXPIRES_TIME_        BIGINT(18) COMMENT '凭证过期时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
...
(
   MATERIAL_ID_         VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(64) COMMENT '名称',
   TYPE_                VARCHAR(128) COMMENT '媒体文件类型',
   FILE_ID_             VARCHAR(64) COMMENT '文件id',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   PRIMARY KEY (MATERIAL_ID_)
);

ALTER TABLE WX_P_MATERIAL COMMENT '非SAAS微信素材';
...
(
   MENU_ID_             VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(64) NOT NULL COMMENT '菜单名称',
   KEY_                 VARCHAR(80) COMMENT '菜单Key',
   PARENT_ID_           VARCHAR(64) NOT NULL COMMENT '上级父ID',
   SN_                  INT COMMENT '序号',
   URL_                 VARCHAR(256) COMMENT '访???地址URL',
   TYPE_                VARCHAR(20) COMMENT '访问方式\r\n',
   TREE_CODE_           VARCHAR(256) COMMENT '树的编码',
   PATH_                VARCHAR(1000) COMMENT 'path',
   CHILDS_              INT,
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
...
(
   TEXT_ID_             VARCHAR(64) NOT NULL COMMENT '主键',
   TO_USER_NAEM__       VARCHAR(256) COMMENT '接收?????户openID',
   FROM_USER_NAME_      VARCHAR(256) COMMENT '发送的用???openID',
   MSG_TYPE_            VARCHAR(256) COMMENT '消息类型',
   CONTENT_             TEXT COMMENT '内容',
   MSGID_               VARCHAR(256) COMMENT '消息id',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   PRIMARY KEY (TEXT_ID_)
);
...
(
   ...
   AGENTID_             VARCHAR(64) COMMENT '代理应用ID',
   USER_ID_             VARCHAR(64) COMMENT '用户ID',
   CONTENT_             TEXT COMMENT '内容',
   STATUS_              SMALLINT COMMENT '状态',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
...
ALTER TABLE WX_P_MSG COMMENT '微信公共号信息';
...
(
   MT_ARTICLE_ID_       VARCHAR(64) NOT NULL COMMENT '主键',
   ARTICLE_ID_          VARCHAR(64),
   THUMB_MEDIA_ID_      VARCHAR(255) COMMENT '素材ID',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   PRIMARY KEY (MT_ARTICLE_ID_)
);

ALTER TABLE WX_P_MT_ARTICLE COMMENT '微信下的文章';
...
(
   MT_MATERIAL_TEMP_ID_ VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(64) COMMENT '名称',
   TYPE_                VARCHAR(128) COMMENT '???体文件类型',
   MEDIA_ID_            VARCHAR(512) COMMENT '媒体id',
   CREATED_AT_          VARCHAR(256) COMMENT '微信创建时间',
   MATERIAL_ID_         VARCHAR(64) COMMENT '文件id',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   PRIMARY KEY (MT_MATERIAL_TEMP_ID_)
);
...
(
   MT_NEWS_ID_          VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(256) COMMENT '名称',
   MEDIA_ID_            VARCHAR(256) COMMENT '媒体文章ID',
   TYPE_                VARCHAR(128) COMMENT '类型',
   CREATED_AT_          VARCHAR(256) COMMENT '微信创建时间',
   NEWS_ID_             VARCHAR(64),
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
...
(
   MT_USER_NEWS_ID_     VARCHAR(64) NOT NULL COMMENT '主键',
   MEDIA_ID_            VARCHAR(256) COMMENT '用于群发的??文消息的media_id',
   USER_NEWS_ID_        VARCHAR(64) COMMENT '群发的消息类型',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   PRIMARY KEY (MT_USER_NEWS_ID_)
);
...
(
   NEWS_ID_             VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(256) COMMENT '??称',
   TYPE_                VARCHAR(128) COMMENT '类型',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   PRIMARY KEY (NEWS_ID_)
);

ALTER TABLE WX_P_NEWS COMMENT '非SAAS微信图文';
...
(
   NEWS_ARTICLE_ID_     VARCHAR(64) NOT NULL COMMENT '主键',
   SN_                  INT COMMENT '序号',
   NEWS_ID_             VARCHAR(64) COMMENT '图文ID',
   ARTICLE_ID_          VARCHAR(64) COMMENT '文章ID',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   PRIMARY KEY (NEWS_ARTICLE_ID_)
);

ALTER TABLE WX_P_NEWS_ARTICLE COMMENT '非SAAS微信图文与文章';
...
ALTER TABLE WX_P_SAAS_ARTICLE COMMENT '微信文章(SAAS功能)';
...
ALTER TABLE WX_P_SAAS_MATERIAL COMMENT '微信素材(SAAS功能)';
...
ALTER TABLE WX_P_SAAS_NEWS COMMENT '微信消息(SAAS功能)';
...
ALTER TABLE WX_P_SAAS_NEWS_ARTICLE COMMENT '微信消息与文章(SAAS功能)';
...
(
   USER_NEWS_ID_        VARCHAR(64) NOT NULL COMMENT '主键',
   NAME_                VARCHAR(256) COMMENT '图文名称',
   IS_MASS_             VARCHAR(64) COMMENT '是否群发',
   TOUSER_              TEXT COMMENT '填写图文消息的接收者',
   FULLNAME_            VARCHAR(64) COMMENT '姓名',
   NEWS_ID_             VARCHAR(64) COMMENT '图文ID',
   MSGTYPE_             VARCHAR(128) COMMENT '群发的消息类型',
   ERRCODE_             VARCHAR(64) COMMENT '错误码',
   STATUS_              SMALLINT COMMENT '状态',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
   CREATE_BY_           VARCHAR(64) COMMENT '创建人ID',
   PRIMARY KEY (USER_NEWS_ID_)
);

ALTER TABLE WX_P_SAAS_USER_NEWS COMMENT '微信用户消息(SAAS功能)';
...
(
   ...
   USER_ID_             VARCHAR(64) COMMENT '用户ID',
   FULLNAME_            VARCHAR(64) COMMENT '姓名',
   WEIXIN_ID_           VARCHAR(64) COMMENT '微信号ID',
   NICK_NAME_           VARCHAR(64) COMMENT '微信用户昵称',
   STATUS_              SMALLINT COMMENT '状态',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
...
ALTER TABLE WX_P_USER COMMENT '微信公众号用户';
...
ALTER TABLE WX_P_USER_NEWS COMMENT '非SAAS微信用户图文';
...
(
   ...
   SQUARE_LOGO_URL_     VARCHAR(512) COMMENT 'SQUARE_LOGO_URL_',
   ROUND_LOGO_URL_      VARCHAR(512) NOT NULL COMMENT 'ROUND_LOGO_URL_',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
   CREATE_TIME_         DATETIME COMMENT '创建时间',
...
ALTER TABLE WX_QY_AGENT COMMENT 'WX_QY_AGENT【微信企业号应用】';
...
(
   ...
   TENANT_NAME_CN_      VARCHAR(256) COMMENT '租户名称',
   APP_TOKEN_           VARCHAR(256) COMMENT '应用凭证',
   CORP_ID_             VARCHAR(256) COMMENT '应用ID',
   CORP_SECRET_         VARCHAR(256) COMMENT '应用密钥',
   ACCESS_TOKEN_        VARCHAR(256) COMMENT '获取到的凭证(访问凭证)',
   EXPIRES_IN_          VARCHAR(256) COMMENT '凭证有效时间，单位：秒',
   EXPIRES_TIME_        BIGINT(18) COMMENT '凭证过期时间',
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
...
ALTER TABLE WX_QY_CONFIG COMMENT 'WX_QY_CONFIG【微信企业号配置】';
...
(
   MENU_ID_             VARCHAR(64) NOT NULL COMMENT '主键',
   AGENT_ID_            VARCHAR(64) COMMENT '代理应用ID',
   NAME_                VARCHAR(64) NOT NULL COMMENT '菜单名称',
   KEY_                 VARCHAR(80) COMMENT '菜单Key',
   PARENT_ID_           VARCHAR(64) NOT NULL COMMENT '上级父ID',
   SN_                  INT COMMENT '序号',
   ...
   PATH_                VARCHAR(1000) COMMENT 'path',
   CHILDS_              INT,
   IS_CUSTOM_           VARCHAR(10) COMMENT '是否为自定义菜单,NO=为固定菜单,YES=为自定义菜单',
   IS_MGR_              VARCHAR(10),
   TENANT_ID_           VARCHAR(64) COMMENT '租用机构ID',
   UPDATE_TIME_         DATETIME COMMENT '更新时间',
   UPDATE_BY_           VARCHAR(64) COMMENT '更新人ID',
...
ALTER TABLE WX_QY_MENU COMMENT 'WX_QY_MENU【微信企业号菜单】';
...
ALTER TABLE WX_QY_MSG COMMENT '微信企业号信息';
...
ALTER TABLE WX_QY_USER COMMENT '微信企业号用户';
...
ALTER TABLE BPM_FORM_VIEW ADD CONSTRAINT BPM_FVIEW_R_SYSTREE FOREIGN KEY (TREE_ID_)
      REFERENCES SYS_TREE (TREE_ID_) ON DELETE SET NULL ON UPDATE RESTRICT;

ALTER TABLE BPM_FV_RIGHT ADD CONSTRAINT BPM_FVR_R_BFV FOREIGN KEY (VIEW_ID_)
      REFERENCES BPM_FORM_VIEW (VIEW_ID_) ON DELETE CASCADE ON UPDATE RESTRICT;
...
ALTER TABLE INS_COLUMN ADD CONSTRAINT COLMN_R_COL_TYPE FOREIGN KEY (TYPE_ID_)
      REFERENCES INS_COL_TYPE (TYPE_ID_) ON DELETE CASCADE ON UPDATE RESTRICT;

ALTER TABLE INS_COL_NEW ADD CONSTRAINT COL_NEW_R_COLMN FOREIGN KEY (COL_ID_)
      REFERENCES INS_COLUMN (COL_ID_) ON DELETE CASCADE ON UPDATE RESTRICT;
...
ALTER TABLE INS_PORT_COL ADD CONSTRAINT PORT_COL_R_INS_COL FOREIGN KEY (COL_ID_)
      REFERENCES INS_COLUMN (COL_ID_) ON DELETE CASCADE ON UPDATE RESTRICT;

ALTER TABLE INS_PORT_COL ADD CONSTRAINT PORT_COL_R_INS_PORAL FOREIGN KEY (PORT_ID_)
      REFERENCES INS_PORTAL (PORT_ID_) ON DELETE CASCADE ON UPDATE RESTRICT;
