...
@XStreamAlias("sysCustomQuery")
public class SysSqlCustomQuery extends BaseTenantEntity {

	/**
	 * 基于Freemark语法来构建sql=freeMakerSql
	 */
	public final  static String BUILD_TYPE_FREEMARKSQL="freeMakerSql";
	/**
	 * 基于Groovy来构建sql=sql
	 */
	public final  static String BUILD_TYPE_GROOVYSQL="sql";
	/**
	 * 基于表来构建SQL=table
	 */
	public final  static String BUILD_TYPE_TABLE="table";
	...
	public String toString() {
		return new ToStringBuilder(this).append("id", this.id).append("name", this.name).append("key", this.key).append("tableName", this.tableName).append("page", this.isPage)
				...
				.append("orderField", this.orderField).append("dsAlias", this.dsAlias).append("table", this.table)
				.append("sqlDiy", this.sqlDiy).append("treeId", this.treeId)
