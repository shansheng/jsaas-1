 ...
 */
@Service
public class OsRelInstManager extends MybatisBaseManager<OsRelInst>{
	@Resource
	...
	public void doChangeInst(String instId,String userId,String groupId){
		...
		if(StringUtils.isNotEmpty(userId)){
			...
			update(osRelInst);
			List<OsRelInst> insts=getByRelTypeIdParty1(osRelInst.getRelTypeId(), orgParty2,osRelInst.getTenantId());
			for(OsRelInst inst:insts){
	...
	public void delInstCascade(OsRelInst inst){
		...
		//取得其下级
		List<OsRelInst> subInsts=getByRelTypeIdParty1(inst.getRelTypeId(), inst.getParty2(),inst.getTenantId());
    ...
    public void deleteByRelTypeId(String relTypeId){
	 ...
	 public List<OsRelInst> getByGroupIdRelTypeId(String groupId,String relTypeId,SqlQueryFilter filter){
		 return osRelInstDao.getByGroupIdRelTypeId(groupId, relTypeId, filter);
	 }
    ...
    public List<OsRelInst> getByRelTypeIdParty1(String relTypeId,String party1,String tenantId){
    	return osRelInstDao.getByRelTypeIdParty1(relTypeId, party1,tenantId);
    }
    ...
    public List<OsRelInst> getByPathRelTypeId(String relTypeId,
			String path) {
    	return osRelInstDao.getByPathRelTypeId(relTypeId, path);
    }
}