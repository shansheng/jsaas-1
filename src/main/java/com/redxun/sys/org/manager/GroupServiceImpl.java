...
public class GroupServiceImpl implements GroupService {
	...
	@Override
	public List<IGroup> getGroupsByUserIdUserType(String userId, String userType) {
		List  groupList= osGroupManager.getByUserIdRelTypeId(userId,OsRelType.REL_CAT_GROUP_USER_BELONG_ID);
		if(StringUtils.isNotEmpty(userType)){
			try{
				OsUserType osUserType=osUserTypeManager.getByCode(userType);
				if(osUserType!=null && 
						StringUtils.isNotEmpty(osUserType.getGroupId())){
					OsGroup group=osGroupManager.get(osUserType.getGroupId());
					if(group!=null){
						groupList.add(group);
					}
				}
	...
	public IGroup getMainByUserId(String userId) {
		IGroup group= osGroupManager.getMainDeps(userId, ContextUtil.getCurrentTenantId());
		return group;
	}
