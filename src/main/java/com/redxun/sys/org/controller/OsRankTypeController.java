...
public class OsRankTypeController extends BaseController{
	...
	@ResponseBody
	public List<OsRankType> listByMyDimId(HttpServletRequest request,HttpServletResponse response) throws Exception{
		String dimId=request.getParameter("dimId");
		List<OsRankType> rankTypeList=osRankTypeManager.getByMyDimIdTenantId(dimId, ContextUtil.getCurrentUserId(), ContextUtil.getCurrentTenantId());
		return rankTypeList;
	}
	...
	public JsonResult saveBatch(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		for(OsRankType osRankType:rankTypes){
			if(osRankType.getLevel()==null){
				try {
					osRankTypeManager.delete(osRankType.getId());
				} catch (Exception e) {
				}
				continue;
			}
			osRankType.setOsDimension(osDimension);
			if (StringUtils.isEmpty(osRankType.getId())) {
				osRankType.setId(IdUtil.getId());
	            osRankTypeManager.create(osRankType);
	        } else {
	        	osRankTypeManager.update(osRankType);
	        }
		}
