...
public class OsRelInstController extends BaseController{
	...
	public ReportLine getReportLines(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(OsRelType.REL_TYPE_GROUP_USER.equals(osRelType.getRelType())){
			...
			reportLine=new ReportLine("0",osRelType.getName(),osRelType.getName()+"关系定义");
			reportLine.setClassName("top-level");
			genReportLines(reportLine,osRelType,tenantId);
		}

		return reportLine;
	}
	
	private void genReportLines(ReportLine reportLine,OsRelType relType,String tenantId){
		
		List<OsRelInst> insts=osRelInstManager.getByRelTypeIdParty1(relType.getId(), reportLine.getId(),tenantId);
		for(OsRelInst inst:insts){
			...
			if(OsRelType.REL_TYPE_USER_USER.equals(relType.getRelType())){
				...
				genReportLines(rl,relType,tenantId);
	...
	public JsonResult updateRelation(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(StringUtils.isNotEmpty(userId)){
			boolean isExist=osRelInstManager.isPartyExistInRelation(osRelInst.getRelTypeId(), userId);
			if(isExist){
				return new JsonResult(true,"你选择的用户在该关系中已经存在，请先移除该用户再更改！");
			}
		}else{
			boolean isExist=osRelInstManager.isPartyExistInRelation(osRelInst.getRelTypeId(), groupId);
			if(isExist){
	...
	public JsonResult saveRelInsts(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		OsRelInst osRelInstCheck=osRelInstManager.get(instId);
		if(StringUtils.isNotEmpty(userIds) && osRelInstCheck !=null){
			boolean isExist=osRelInstManager.isPartyExistInRelation(osRelInstCheck.getRelTypeId(), userIds);
			if(isExist){
				return new JsonResult(true,"你选择的用户在该关系中已经存在，请先移除该用户再更改！");
			}
		}
	...
	public JsonResult joinTwoGroupUsers(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		String dimId1=null;
		if(mainGroup!=null&& mainGroup.getDimId()!=null){
			dimId1=mainGroup.getDimId();
		}
		OsGroup subGroup=osGroupManager.get(p2);
		
		String dimId2=null;
		if(subGroup!=null&& subGroup.getDimId()!=null){
			dimId1=subGroup.getDimId();
		}
		...
		for(String uId:uIds){
			...
			if(osRelInst==null){
				...
				inst.setIsMain(MBoolean.NO.name());
				inst.setStatus(MStatus.ENABLED.name());
				inst.setRelTypeId(belongRelType.getId());
	...
	public List<OsRelInst> listByUserIdExcludeBelong(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		for(OsRelInst inst:relInsts){
			if(StringUtils.isNotEmpty(inst.getRelTypeId())) {
				OsRelType type = osRelTypeManager.get(inst.getRelTypeId());
				inst.setRelTypeName(type.getName());
			}
