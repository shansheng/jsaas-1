...
public class OsDimensionController extends TenantListController{
	...
	@RequestMapping("gradeJsonAll")
	@ResponseBody
	public List<OsDimension> gradeJsonAll(HttpServletRequest request,HttpServletResponse response){
		QueryFilter queryFilter=QueryFilterBuilder.createQueryFilter(request);
		queryFilter.addFieldParam("DIM_ID_", QueryParam.OP_IN, "1,2");
		queryFilter.addSortParam("DIM_ID_", "asc");
		List<OsDimension> dimList=osDimensionManager.getAll(queryFilter);
		return dimList;
	}
	...
	public List<OsDimension> jsonAll(HttpServletRequest request,HttpServletResponse response){
		...
		String dimId=request.getParameter("dimId");
		QueryFilter queryFilter=QueryFilterBuilder.createQueryFilter(request);
		if(StringUtil.isNotEmpty(dimId)){
			queryFilter.addFieldParam("DIM_ID_",dimId);
		}
