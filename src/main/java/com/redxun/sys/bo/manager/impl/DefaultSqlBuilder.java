...
public class DefaultSqlBuilder implements ISqlBuilder {
	...
	public SqlModel getByFk(SysBoEnt boEnt, String fk) {
		...
		String sql="select * from " +  boEnt.getTableName()   +" where " + fkField + "=#{fk}";
	...
	private void handUser(SqlModel sqlModel){
		...
		String sql= sqlModel.getSql() + " and " + SysBoEnt.FIELD_CREATE_BY +"=#{"+SysBoEnt.FIELD_CREATE_BY+"}";
		
		sqlModel.addParam(SysBoEnt.FIELD_CREATE_BY, userId);
