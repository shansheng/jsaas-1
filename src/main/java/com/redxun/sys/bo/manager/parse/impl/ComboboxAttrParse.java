	...
	public JSONObject getInitData(SysBoAttr attr) {
		...
		AttrParseUtil.addKey(jsonObject, obj.getString("defaultvalue"));
		 ...
		 if(StringUtil.isNotEmpty(defaultvalue)){
		 	String name =null;
		 	try{
				com.alibaba.fastjson.JSONArray jsonArray = obj.getJSONArray("data");
				for(int i=0;jsonArray !=null &&i<jsonArray.size();i++){
					 if(jsonArray.getJSONObject(i) !=null && defaultvalue.equals(jsonArray.getJSONObject(i).getString("key"))){
						 name = jsonArray.getJSONObject(i).getString("name");
						 break;
					 }
				}
				if(StringUtil.isNotEmpty(name)){
					AttrParseUtil.addName(jsonObject,name);
				}
			}catch (Exception e){
			}
		 }
