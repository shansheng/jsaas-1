...
public class SchedulerController extends GenericController {
	...
	public JsonResult addJob(HttpServletResponse response, HttpServletRequest request) throws Exception {
		...
		try {
			...
			String update = RequestUtil.getString(request, "update","");
			...
			if (isExist && "".equals(update)) {
				...
				if(StringUtil.isEmpty(update)){
					...
					obj = new JsonResult(true, "更新任务成功");
				}
			}
	...
	@ResponseBody
	public JobInfo jobInfo(HttpServletResponse response, HttpServletRequest request) throws SchedulerException {
		String name=RequestUtil.getString(request, "name");
		JobInfo jobInfo=schedulerService.getJobByName(name);
		return jobInfo;
	}
