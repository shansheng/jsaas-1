...
public class SysTreeController extends BaseListController {
	...
	public List<SysTree> listBpmSolution(HttpServletRequest request, HttpServletResponse response) throws Exception {
		List<SysTree> treeList = sysTreeManager.getByCatKeyTenantId("CAT_BPM_SOLUTION",
	...
	public JsonResult saveFolder(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String msg = null;
		List<SysTree> tree = sysTreeManager.getByCatKeyTenantId(sysTree.getCatKey(), ContextUtil.getCurrentTenantId());
		for (SysTree sysTree2 : tree) {
			if (sysTree.getParentId().equals(sysTree2.getParentId()) && name.equals(sysTree2.getName())) {
				msg = "文件夹目录名称：" + sysTree2.getName() + " 已经存在!";
	 ...
	 * JSONUtil.getString(jsonObj,"children"); if(StringUtils.isNotEmpty(children)){
	 * genKmsType(children,sysTree); } if(isCreated){
	 * sysTreeManager.create(sysTree); }else{ sysTreeManager.update(sysTree); } } }
	 */
	...
	@ResponseBody
	public JSONObject createFormSolutionTree(@PathVariable("paramKey") String paramKey, HttpServletResponse response,
	...
	@RequestMapping({ "delBpmTem" })
	@ResponseBody
	public JsonResult delBpmTem(HttpServletRequest request, HttpServletResponse response) {
		String pkId = request.getParameter("pkId");
		List<SysTree> sysTrees = this.sysTreeManager.getByParentId(pkId);
		for (SysTree sysTree : sysTrees) {
			this.sysTreeManager.deleteObject(sysTree);
		}
		String parentId = ((SysTree) this.sysTreeManager.get(pkId)).getParentId();
		if (StringUtils.isNotEmpty(parentId)) {
			SysTree sysTreeFather = (SysTree) this.sysTreeManager.get(parentId);
			if ((sysTreeFather.getChildren() != null) && (sysTreeFather.getChildren().size() > 0)) {
				this.sysTreeManager.saveOrUpdate(sysTreeFather);
			}
		}
		this.sysTreeManager.delete(pkId);
		return new JsonResult(true, "删除成功！");
	}

	@RequestMapping({ "templateEdit" })
	public ModelAndView templateEdit(HttpServletRequest request, HttpServletResponse response) {
		String pkId = request.getParameter("pkId");
		String parentId = request.getParameter("parentId");
		SysTree sysTree = null;
		String haveSonOrNot = "";
		...
		String isAll = "";
		if (StringUtils.isNotEmpty(pkId)) {
			sysTree = (SysTree) this.sysTreeManager.get(pkId);
			List<SysTree> sysTrees = this.sysTreeManager.getByParentId(pkId);
			if (sysTrees.size() > 0) {
				haveSonOrNot = "YES";
			}
			BpmSolTemplateRight bpmSolTemplateRight = this.bpmSolTemplateRightManager.getByTreeId(pkId);

			StringBuffer userNamesBuffer = new StringBuffer("");
			StringBuffer groupNamesBuffer = new StringBuffer("");
			if (bpmSolTemplateRight != null) {
				String userIds = bpmSolTemplateRight.getUserIds();
				isAll = bpmSolTemplateRight.getIsAll();
				String singleName;
				if (StringUtils.isNotBlank(userIds)) {
					TuserIds = userIds;
					String[] userIdArray = userIds.split(",");
					for (String string : userIdArray) {
						singleName = ((OsUser) this.osUserManager.get(string)).getFullname();
						userNamesBuffer.append(singleName);
				...
				if (StringUtils.isNotBlank(groupIds)) {
					...
					for (String string : groupIdArray) {
						OsGroup osgroup = (OsGroup) this.osGroupManager.get(string);
						if (osgroup != null) {
							singleName = osgroup.getName();
							groupNamesBuffer.append(singleName);
							groupNamesBuffer.append(",");
						}
					}
					groupNames = groupNamesBuffer.toString();
			...
			sysTree = new SysTree();
			sysTree.setCatKey("BPM_TEMPLATE_TREE");
			sysTree.setParentId(parentId);
		}
		String nameOfSol = "";
		String bindSolId = sysTree.getBindSolId();
		if (StringUtils.isNotEmpty(bindSolId)) {
			BpmSolution sol = (BpmSolution) this.bpmSolutionManager.get(bindSolId);
			if (sol != null) {
				nameOfSol = sol.getName();
			}
		}
		return getPathView(request).addObject("sysTree", sysTree).addObject("nameOfSol", nameOfSol)
				.addObject("haveSonOrNot", haveSonOrNot).addObject("userIds", TuserIds)
				.addObject("userNames", userNames).addObject("groupIds", TgroupIds).addObject("groupNames", groupNames)
				.addObject("isAll", isAll);
	}
