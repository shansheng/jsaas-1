...
public class SysReportFormController extends BaseFormController {
	...
	public JSONObject uploadTemplate(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		String treeId = request.getParameter("treeId");//分类ID
		String sourceId = request.getParameter("sourceId");//数据源ID
		String repId = request.getParameter("repId");//报表ID
		String subject = request.getParameter("subject");//标题
		String key = request.getParameter("key");
		String descp = request.getParameter("descp");//已经取消
		String paramConfig=request.getParameter("paramConfig");//传入参数控件的配置化json,格式[{"fc":"mini-textbox","fcName":"文本框","_id":1,"_uid":1,"_state":"added","fieldLabel":"姓名","fieldName":"name"}]
		//String bean = request.getParameter("selfHandleBean");
		String tenantId = ContextUtil.getCurrentTenantId();
		JSONObject rtnObj=new JSONObject();//将用于返回的jsonObject
		...
		StringBuffer sbPath = new StringBuffer();//报表保存的路径
