...
public class SysTreeDao extends BaseJpaDao<SysTree> {
     ...
     * @return
     */
    public List<SysTree> getByPath(String path){
    	String ql="from SysTree where path like ?";
    	return this.getByJpql(ql, new Object[]{path+"%"});
    }
    ...
    public List<SysTree> getByUserIdCatKey(String userId,String catKey){
    	String ql="from SysTree st where st.userId=? and st.catKey=? order by st.sn asc";
    ...
    public SysTree getByKeyNotCurrent(String treeId, String key,String catKey,String tenantId) {
    	String jpql="from SysTree where key=? and catKey=? and tenantId=? and treeId!=?";
    	return (SysTree) getUnique(jpql, key,catKey,tenantId,treeId);
    }
    ...
    public List<SysTree> getByTreeIdNameCatKey(String tenantId,String path,String catKey){
    	List<Object> params=new ArrayList<Object>();
    	String ql="from SysTree tree where tree.tenantId in (?,?)";
    	params.add(tenantId);
    	params.add(ITenant.PUBLIC_TENANT_ID);
    	...
    	if(StringUtils.isNotEmpty(catKey)){
    		ql+=" and tree.catKey like ?";
    		params.add("%"+ catKey + "%");
    	}
    	return this.getByJpql(ql, params.toArray());
    }

	public List<SysTree> getByAttributeValueTargetId(String userId) {
		String ql="from SysTree st where st.treeId in (select o.treeId from OsCustomAttribute o where o.id in(select a.attributeId from OsAttributeValue a where a.targetId = ?))";
