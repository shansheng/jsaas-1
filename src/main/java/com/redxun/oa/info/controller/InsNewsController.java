...
public class InsNewsController extends TenantListController {
	...
	@ResponseBody
	public JsonResult deploy(@RequestBody JSONObject jsonObj){
		
		String columnId=jsonObj.getString("columnId");
		String newIds=jsonObj.getString("newId");
		
		String[] aryNewId=newIds.split("[,]");
		for(String newId:aryNewId ){
			insNewsQueryDao.updColumn(newId, columnId);
		}
	 ...
	 * 分页返回查询列表
	 */
	@RequestMapping("listDataJson")
	public void listDataJson(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(StringUtils.isNotEmpty(export)){
			...
			for (Object object : list) {
				InsNews insNews = (InsNews) object;
				if(StringUtil.isNotEmpty(insNews.getColumnId())) {
					InsColumnDef def=insColumnDefManager.get(insNews.getColumnId());
					if(def!=null){
						insNews.setColumnName(def.getName());
					}
				}
			}
	...
	public ModelAndView get(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotBlank(pkId)) {
			...
			if(StringUtil.isNotEmpty(insNews.getColumnId())) {
				InsColumnDef colDef=insColumnDefManager.get(insNews.getColumnId());
				if(colDef!=null){
					insNews.setColumnName(colDef.getName());
				}
			}
		} else {
			insNews = new InsNews();
