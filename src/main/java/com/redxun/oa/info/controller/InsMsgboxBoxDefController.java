...
public class InsMsgboxBoxDefController extends BaseMybatisListController{
    ...
    @ResponseBody
    public JsonPageResult/*List<InsMsgboxBoxDef>*/ getByBoxId(HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	String boxId = RequestUtil.getString(request, "boxId");
    	List<InsMsgboxBoxDef> boxs = insMsgboxBoxDefManager.getByBoxId(boxId);
    	int listSize = boxs.size();
    	
    	QueryFilter queryFilter = getQueryFilter(request);
    	int pageSize = queryFilter.getPage().getPageSize();
    	int pageIndex = queryFilter.getPage().getPageIndex();
    	int lastIndex = (pageSize * (pageIndex + 1));
    	...
    	if(pageIndex == 0) {
    		...
    		if((pageIndex + 1) * pageSize >= listSize) {
    			lastIndex = listSize;
    		}
    	}
    	
    	boxs = boxs.subList(pageSize * pageIndex, lastIndex);
    	for(InsMsgboxBoxDef box:boxs){
    		InsMsgDef msg = insMsgManager.get(box.getMsgId());
    		if(msg!=null){
    			box.setContent(msg.getContent());
    	...
    	JsonPageResult<?> result=new JsonPageResult(boxs, listSize);
