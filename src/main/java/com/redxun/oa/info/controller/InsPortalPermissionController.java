    ...
    public ModelAndView edit(HttpServletRequest request,HttpServletResponse response) throws Exception{
    	...
    	for(InsPortalPermission list:lists){
    		...
    		if(InsPortalPermission.TYPE_USER.equals(type)){
    		    OsUser osUser = osUserManager.get(list.getOwnerId());
    		    if(osUser!=null){
                    list.setOwnerName(osUser.getFullname());
                }
    		}else if(InsPortalPermission.TYPE_GROUP.equals(type)||InsPortalPermission.TYPE_SUBGROUP.equals(type)){
                OsGroup osGroup = osGroupManager.get(list.getOwnerId());
                if(osGroup!=null){
                    list.setOwnerName(osGroup.getName());
                }
