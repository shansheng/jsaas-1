...
public class ProItemFormController extends BaseFormController {
    ...
    public JsonResult save(HttpServletRequest request, @ModelAttribute("proItem") @Valid ProItem proItem, BindingResult result) {
    	boolean isKeyExist=proItemManager.isKeyExist(proItem.getAlias(),ContextUtil.getCurrentTenantId(),proItem.getID());
    	if(isKeyExist){
    		result.rejectValue("alias", "alias.exist", "标识键："+proItem.getAlias()+"已经存在！");
    	}
