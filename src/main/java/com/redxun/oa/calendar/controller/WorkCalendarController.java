...
public class WorkCalendarController extends BaseListController{
	...
	public JSONObject drawCalendar(HttpServletRequest request,HttpServletResponse response){
		String tenantId=ContextUtil.getCurrentTenantId();
		String start=request.getParameter("start");//fullcalendar传来的开始时间错
		String end=request.getParameter("end");//结束时间戳
		String timeBlockValue=request.getParameter("timeBlockValue");//日历使用的时间区间设定
		String whereCross="";
		...
		String name=workTimeBlock.getSettingName();//时间区间的命名
		String calSettingId=request.getParameter("calSettingId");//日历设定
		...
		for (Map.Entry<String, Long> entry : stamp.entrySet()) {//调整map的时间
			   String key=entry.getKey();
			   if(whereCross.contains("endTime")){//假如包含原始数据,即第一条费plus的数据
