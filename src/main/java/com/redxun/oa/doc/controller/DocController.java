...
public class DocController extends TenantListController {
	...
	public ModelAndView edit(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (SysFile file : doc.getInfDocFiles()) {// 把每个doc附件都遍历并用逗号分隔拼接成ID和Name字符串用来显示
			...
			json.put("fileId", file.getFileId());
			json.put("fileName", file.getFileName());
			json.put("totalBytes", file.getTotalBytes());
			files.add(json);
		}
		...
		String docPath=doc.getDocPath();
		return mv.addObject("doc", doc).addObject("files", files.toString()).addObject("docPath",docPath).addObject("docType", docType);
	}
