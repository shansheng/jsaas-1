...
public class BpmController {
	...
	private Integer getDraftCount(){
		String userId=ContextUtil.getCurrentUserId();
		
		Integer counts=bpmInstManager.getMyInstCount(userId,"DRAFTED");
		return counts;
	}
	...
	public ListModel myAttendsData(HttpServletRequest request) throws Exception{
		...
		List<BpmNodeJump> bpmInstList=bpmNodeJumpDao.getMyCheckInst(filter);
	...
	private List<FormModel> getStartMobileForms(String solId,String instId){
		...
		for(BpmMobileForm mobileForm:mobileList){
			...
			formModel.setTimeStamp(mobileForm.getUpdateTime()==null?"":String.valueOf(mobileForm.getUpdateTime().getTime()));
			list.add(formModel);
		}
	...
	private List<FormModel> getMobileForms(String nodeId,BpmInst bpmInst,Map<String,Object > vars){
		...
		for(BpmMobileForm mobileForm:mobileList){
			...
			formModel.setJsonData(dataJson);
			//添加时间戳
			if(mobileForm.getUpdateTime()!=null){
	...
	private Map<String,String> getDetailPermission(String solId,String actDefId,List<FormModel> formModels){
		...
		Map<String,String> map=new HashMap<String, String>();
		for(FormModel formModel:formModels){
			JSONObject rightSetting=FormUtil.getByBoDetail(formModel.getFormKey(),  actDefId, solId , formModel.getBoDefId());
			JSONObject rightJson=bpmFormRightManager.calcRights(rightSetting, profileMap, false);
			map.put(formModel.getBoDefId(), rightJson.toJSONString());
		}
		return map;
    ...
    @RequestMapping("registCid")
 	@ResponseBody
    public JsonResult registCid(HttpServletRequest request,HttpServletResponse response) throws Exception{    	
    	String cid = RequestUtil.getString(request, "cid");
    	String mobileType = RequestUtil.getString(request, "mobileType");
    	IUser user = ContextUtil.getCurrentUser();
    	String userId = "";
    	if(user!=null){
    		userId = user.getUserId();
    	}
    	if(StringUtil.isEmpty(cid))  return new JsonResult(false, "请传入CID");
    	if(StringUtil.isEmpty(mobileType)) return new JsonResult(false, "请传入MOBILETYPE!");
		
		QueryFilter filter = QueryFilterBuilder.createQueryFilter(request);
		filter.addFieldParam("cid", cid);
		filter.addFieldParam("mobileType", mobileType);   			
		Long totalCount = bpmMobileTagManager.getTotalItems(filter);	
		if(totalCount==null||totalCount==0){
			BpmMobileTag bpmMobileTag = new BpmMobileTag();
			bpmMobileTag.setTagid(IdUtil.getId());
			bpmMobileTag.setAlias(userId);
			bpmMobileTag.setCid(cid);
			bpmMobileTag.setMobileType(mobileType);
			bpmMobileTagManager.create(bpmMobileTag);
		}
     ...
     * title 消息标题
     * text 消息内容
     * body 透传消息，json字符串，后台可通过此json传送数据到手机端， 格式为{"action":"手机的路由名称","test":"aaa",...} 其中action键值必须设置    
     */
    @RequestMapping("pushMsgToUser")
    @ResponseBody
    public JsonResult pushMsgToUser(HttpServletRequest request,HttpServletResponse response) throws Exception{
		String targetUserList = RequestUtil.getString(request, "targetUserList");
		...
		PushMobileMsgUtil.pushMsgToUser(targetUserList, title, text, body);
