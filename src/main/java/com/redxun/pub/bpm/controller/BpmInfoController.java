	...
	public JSONArray getTaskInfos(HttpServletRequest request,HttpServletResponse respnse) throws Exception{
		...
		for(String instId:aryInstId){
			...
			for(Task td:curTasks){
				...
				Collection<TaskExecutor> taskIdentites=bpmTaskManager.getTaskHandleIdentityInfos(td.getId());
				StringBuffer userIds=new StringBuffer();
				StringBuffer uNames=new StringBuffer();
				
				//构造执行用户
				for(TaskExecutor info:taskIdentites){
					if(TaskExecutor.IDENTIFY_TYPE_USER.equals(info.getType())){
						//为当前用户，则加上
						if(info.getId().equals(curUserId)){				
						...
						uNames.append(info.getName());
					}else {
						List<OsUser> osUsers=osUserManager.getByGroupIdRelTypeId(info.getId(), OsRelType.REL_CAT_GROUP_USER_BELONG_ID);
						for(OsUser osUser:osUsers){
							...
							userIds.append(osUser.getUserId());
							uNames.append(osUser.getFullname());
						}
					}
				}
