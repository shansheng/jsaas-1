 ...
 */
@Service
public class WxEntOrgManager extends MybatisBaseManager<WxEntAgent>{
	@Resource
	...
	private JsonResult<TokenModel>  getToken(String tenantId){
		...
		WxEntCorp corp= wxEntCorpDao.getByTenantId(tenantId);
		try{
	...
	public JsonResult<List<String>> syncOrg(String tenantId,List<String> userIds) throws Exception{
		...
		for(WxOrg org:orgList){
			...
			if(jsonResult.isSuccess()){
				osGroupDao.updWxFlag(org.getOriginId());
			}
			else{
	...
	private List<WxOrg> buidOrgs(String tenantId){
		List<OsGroup> orgs= osGroupDao.getSyncWx(tenantId);
	...
	private void handWxId(List<OsGroup> rtnOrgs,Map<String,OsGroup> groupMap){			
		...
		for(OsGroup group:rtnOrgs){				
			...
			Integer maxWxpid = osGroupDao.getMaxPid();
			if(maxWxpid==null){
				maxWxpid = 100;
			}
			...
			osGroupDao.updateWxPid(group.getGroupId(), wxParentPid, wxPid);
			tmpOrgs.add(group);
	...
	private List<WxUser> buidUsers(String tenantId,List<String> userIds){
		List<OsUser> userList=osUserDao.getSyncWx(tenantId);
