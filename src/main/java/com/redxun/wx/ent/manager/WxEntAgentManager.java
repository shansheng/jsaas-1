 ...
 */
@Service
public class WxEntAgentManager extends MybatisBaseManager<WxEntAgent>{
	@Resource
	...
	public void create(WxEntAgent entity) {
		...
		if(corp==null){
			...
			wxEntCorpDao.create(entity.getWxEntCorp());
		}
		//更新
		else{
	...
	public WxEntAgent getByAgent(String agentId){
		String tenantId=ContextUtil.getCurrentTenantId();
		WxEntAgent agent= this.wxEntAgentDao.getByAgent(tenantId, agentId);
		return agent;
	}
	...
	public WxEntAgent getByTenantId(String tenantId){
		List<WxEntAgent> agents= this.wxEntAgentDao.getByTenantId(tenantId);
