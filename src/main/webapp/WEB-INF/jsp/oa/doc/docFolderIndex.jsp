...
<ul id="rightClickFileMenu" class="mini-menu" style="display:none;">
	<li onclick="addDocOnThat('html')">Html文档</li>
	<li  onclick="addDocOnThat('doc')">Word文档</li>
	<li  onclick="addDocOnThat('xls')">Excel文档</li>
	<li  onclick="addDocOnThat('ppt')">PowerPoint文档</li>
</ul>
...
<div id="toolbar1" class="mini-toolbar" >
	<div class="form-toolBox">
		<ul>
			...
			<li><input class="text text-border" id="key" name="key" style="vertical-align: middle;height: 30px;border: 0;border: 1px solid #ddd;"/></li>
...
<div class="mini-fit rx-grid-fit">
	<div id="datagrid1" class="mini-treegrid" style="width:100%;height:100%;"    showTreeLines="true"
		 showTreeIcon="true"
		 treeColumn="name" idField="dafId" parentField="parent" resultAsTree="false"  ondrawnode="onDrawNode"
		 onnodedblclick="openNew()"
		...
		<div property="columns">
			...
			<div  name="name" field="name" width="160" headerAlign=""  align="left">文档名称</div>
			<div name="createName" field="createName" width="60" headerAlign="" >创建人</div>
			<div name="createTime" field="createTime"  dateFormat="yyyy-MM-dd" width="60" headerAlign="" >创建时间</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
  ...
  */
	...
	function onActionRenderer(e){
		...
		var s='';
		s+= ' <span  title="编辑" onclick="editMyRow()">编辑</span>';
		s+= ' <span  title="删除" onclick="delMyRow()">删除</span>';
		s+= '<span   title="明细" onclick="detailMyRow()">明细</span>';
		if(type=="文件夹"){
			s+= ' <span  title="新建文件夹" onclick="addrowfolder()">新建文件夹</span>';
			s+= ' <span  title="新建文件" onclick="addDocOnThis(event)">新建文件</span>';
		}else{
			//文件则增加移动到按钮
			s+=' <span  title="移动" onclick="moveTo()">移动</span>';}
