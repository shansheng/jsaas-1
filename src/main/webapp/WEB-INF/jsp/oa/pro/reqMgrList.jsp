...
<body>
	<div id="toolbutton1" class=" form-outer">
        <redxun:toolbar entityName="com.redxun.oa.pro.entity.ReqMgr" excludeButtons="popupExportMenu1,popupAddMenu,popupAttachMenu,detail,edit,remove,export,importData,popupSearchMenu,fieldSearch,popupSettingMenu" >
		<div class="self-toolbar">
		 <li>
		 <a class="mini-button"  onclick="addOne()" plain="true">增加需求</a>
		 </li>
		 <li>
		 <a class="mini-button" plain="true" onclick="location.reload();">刷新</a>
		 </li>
		 <li>
		 <a class="mini-button"  plain="true" href= "${ctxPath}/oa/pro/project/generateToWord.do?projectId=${projectId}" target="_blank">生成word</a>
		 </li>
		</div>
      </redxun:toolbar>
     </div>
	 <div class="mini-fit form-outer" style="height: 100%;">
		...
		>
			<div property="columns">
				<div name="action" cellCls="actionIcons" width="60" headerAlign="center" align="center" renderer="onActionRenderer"
         ...
         _LoadUserInfo();//加载user进去单元格
	        ...
	        function onActionRenderer(e) {
                ...
                if(record.status=='DRAFT'||record.status=='NOTPASS'){//如果是草稿或者是没通过的需求才可以进行编辑
                    s+= ' <span  title="编辑" onclick="editOne(\'' + pkId + '\')">编辑</span>';
                }
                if(record.status=='DRAFT'||record.status=='NOTPASS'){
    	            s+= ' <span  title="删除" onclick="delRow(\'' + pkId + '\')">删除</span>';
                }
	                s+= ' <span  title="增加子需求" onclick="addSubReq(\'' + pkId + '\')">增加子需求</span>';
	            return s;
	        }
