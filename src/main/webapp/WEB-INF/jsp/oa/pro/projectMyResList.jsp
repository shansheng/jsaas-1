...
<body>
	<redxun:toolbar entityName="com.redxun.oa.pro.entity.Project" excludeButtons="popupAddMenu,popupAttachMenu,detail,edit,remove,,popupSettingMenu,export,importData">
			</redxun:toolbar>
			...
			<div class="mini-fit rx-grid-fit" style="height: 100%;">
				<div id="datagrid1" class="mini-datagrid"
					...
					<div property="columns">
						...
						<div field="reporId" width="120" >负责人</div>
						<div field="status" width="120"  renderer="onStatusRenderer">状态</div>
						<div field="version" width="120" >当前版本</div>
						<div field="type" width="120"  renderer="onTypeRenderer">类型</div>
						<div field="startTime" width="120"  renderer="onRenderer">启动时间</div>
						<div field="endTime" width="120"  renderer="onRenderer">结束时间</div>
					</div>
				</div>
			</div>
	...
	mini.parse();
		...
		function onActionRenderer(e) {
			var record = e.record;
			var uid = record.pkId;
			var  s = '<span  title="明细" onclick="detailMyRow(\''+uid+ '\',true)">明细</span>';
			     s += ' <span  title="编辑" onclick="editMyRow(\''+ uid+ '\',true)">编辑</span>';
		...
		//项目明细
		function detailMyRow(pkId,max) {
	        _OpenWindow({
	        	url: __rootPath + "/oa/pro/project/get.do?pkId=" + pkId,
	            title: "项目明细", width: 760, height: 820,max:max
	        });
	    }
		
		
		//编辑行数据
	    function editMyRow(pkId,max) {        
	        _OpenWindow({
	    		 url: __rootPath + "/oa/pro/project/edit.do?pkId="+pkId,
	            title: "编辑项目",
	            width: 960, height: 730,max:max,
	            ondestroy: function(action) {
	                if (action == 'ok') {
	                    grid.reload();
	                }
