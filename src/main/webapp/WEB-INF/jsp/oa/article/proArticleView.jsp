...
<body>
	...
	<div class="mini-splitter" style="width:100%;height:100%;" allowResize="true">
	    <div size="20%" showCollapseButton="true">
			...
			<div class="mini-fit">
				<ul id="tree1" class="mini-tree" url="${ctxPath}/oa/article/proArticle/getAllByItemId.do?itemId=${proId}" style="width: 400px;height:100%; padding: 5px;" showTreeIcon="true" textField="title"
				idField="id"  parentField="parentId" allowDrag="true"  allowDrop="true" onnodeclick="changeFrame" onbeforedrop="onbeforedrop" contextMenu="#treeMenu" resultAsTree="false" expandOnLoad="0" ondrawnode="onDrawNode">
				</ul>
			</div>
		</div>
	...
	<ul id="treeMenu" class="mini-contextmenu" onbeforeopen="onBeforeOpen" >
		...
		<li name="remove" iconCls="icon-remove" class=" btn-red" onclick="onRemoveNode">删除</li>
		...
		<li name="shareOnLine" iconCls="icon-downgrade" onclick="shareOnLine">分享链接</li>
	</ul>
	...
	function shareOnLine(e){
		...
		var nodeId = node.id;
		if(node.id==0){
			url="${ctxPath}/oa/article/proItem/shareOnLine.do?proId="+${param.pkId};
		}
		else{
			var url="${ctxPath}/oa/article/proItem/shareOnLine.do?pkId="+nodeId;	
		}
	...
	function changeFrame(e){
		...
		mini.confirm("确定移动到此处?", "提示",
	            function (action) {
                	...
                	var url="${ctxPath}/oa/article/proArticle/moveArticle.do";
                	$.post(url,{targetId:node.id,nodeId:preClickNodeId},function(data){
                		mini.showTips({
	...
	function onAddNode(e){
		 ...
		 newNode.id="";
		 newNode.title="新文章";
		 newNode.belongProId="${proId}";
		 tree.addNode(newNode,0,node);
		 $("#contentFrame").attr("src","${ctxPath}/oa/article/proArticle/edit.do?parentId="+node.id+"&itemId=${proId}");
	...
	function previewHtml(){
		var url="${ctxPath}/oa/article/proArticle/previewHtml/${proId}.do";
		$.get(url,function(result){
			window.open( "${ctxPath}/doc/${alias}/index.html");
		})
	}
	...
	*/
	...
	<redxun:gridScript gridId="datagrid1" entityName="com.redxun.oa.article.entity.ProArticle" winHeight="450"
		winWidth="700" entityTitle="项目" baseUrl="oa/article/proArticle" />
</body>
</html>