	...
	<div id="layout1" class="mini-layout" style="width: 100%; height: 100%;">
		...
		<div region="west" showSplit="true" showHeader="false" width="200">
			...
			>
				...
				<a class="mini-button"  plain="true" onclick="writeMail" tooltip="写邮件">写邮件</a>
				<a class="mini-button" plain="true" onclick="refreshFolder" tooltip="刷新">刷新</a>
			</div>
			...
			<ul id="treeMenu" class="mini-contextmenu" onbeforeopen="onBeforeOpen">
				<li name="brother"  onclick="addBrotherFolder">新增同级文件夹</li>
				<li name="children"  onclick="addChildrenFolder">新增子文件夹</li>
				<li name="edit"  onclick="editFolder">编辑文件夹</li>
				<li name="remove"  class=" btn-red" onclick="delFolder">删除文件夹</li>
			</ul>
		</div><!-- end of west -->
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			if (!tab) {
				...
				tab.url = "${ctxPath}" + "/oa/mail/innerMail/list.do?folderId="
						+ recNode.folderId+"&isShowDel="+isShowDel+"&type=RECEIVE-FOLDER"; 
				tabs.addTab(tab);
				tabs.activeTab(tab);
			}
			else{
		...
		function showTab(node) {
			...
			if (!tab) {
				...
				tab.url = "${ctxPath}" + "/oa/mail/innerMail/list.do?folderId="
						+ node.folderId+"&isShowDel="+isShowDel+"&type="+node.type; 
				tabs.addTab(tab);
				tabs.activeTab(tab);
			}
			else{
