...
<body>
	<rx:toolbar toolbarId="toolbar1" pkId="${mailConfig.configId}" hideRecordNav="true" hideRemove="true" excludeButtons="remove" >
		<div class="self-toolbar">
			<a class="mini-button" plain="true" onclick="testConnect">连接</a>
			<c:choose>
				<c:when test="${mailConfig.configId!=null&&mailConfig.configId!=''}">
					<a class="mini-button btn-red"  plain="true" onclick="onRemove">删除</a>
				</c:when>
			   <c:otherwise>
			   		<a class="mini-button btn-red" plain="true"  visible="false" onclick="onRemove">删除</a>
			   </c:otherwise>
			</c:choose>
		</div>
	</rx:toolbar>
