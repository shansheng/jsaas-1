...
<body>
	<div id="layout1" class="mini-layout" style="width:100%;height:100%;">
	 	...
	 	>
		 	<input 
			    ...
			    url="${ctxPath}/sys/org/osDimension/jsonAll.do?containContact=yes&tenantId=<c:out value="${param.tenantId}&dimId=${adminDim.dimId}" />"
			    textField="name" 
			    valueField="dimId" 
		    ...
		    /> 
		 	<div 
		 		id="groupTree" 
		 		class="mini-tree"  
		 		url="${ctxPath}/sys/org/sysOrg/listByDimId.do?tenantId=<c:out value="${param.tenantId}" />&dimId=${adminDim.dimId}&initRankLevel=${initRankLevel}"
		 		style="width:100%;" 
				showTreeIcon="true"  
				resultAsTree="false"
				textField="name" 
		  ...
		  <c:if test="${param['single']==false}">
				<div region="east" title="选中用户列表" width="250" showHeader="false" showCollapseButton="false">
					<div class="mini-toolbar mini-toolbar-one">
						<a class="mini-button btn-red" iconCls="icon-remove" onclick="removeSelectedUser">移除</a>
						<a class="mini-button btn-red" iconCls="icon-trash"  onclick="clearUser">清空所有</a>
					</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			//指定等级
			if(${!empty initRankLevel}){
				mini.get('dimCombo').setVisible(false);
			}
