...
<body>
         ...
         <ul>
			<li>
				<a class="mini-button"   onClick="addTrigger">新增计划</a>
			</li>
			<li>
				<a class="mini-button btn-red"   onclick="getJog()">返回</a>
			</li>
	 ...
	 <div class="mini-fit" style="height: 100%;">
		<div id="datagrid1" class="mini-datagrid"
			...
			allowAlternating="true"  emptyText="请新增计划.." showEmptyText="false">
	...
	<script type="text/javascript">
		...
		function onActionRenderer(e) {
			...
			if(state == "NORMAL"){
				result+='<span onclick="toggleTrigger(\''+triggerName+'\')"   title="禁用">禁用</span>';
			}
			if(state == "PAUSED"){
				result+='<span onclick="toggleTrigger(\''+triggerName+'\')"   title="启用">启用</span>';
			}
			result+='<span onclick="getLog(\''+triggerName+'\')"   title="日志">日志</span>';
			
			result+='<span onclick="removeTrigger(\''+triggerName+'\')"   title="删除">删除</span>';
