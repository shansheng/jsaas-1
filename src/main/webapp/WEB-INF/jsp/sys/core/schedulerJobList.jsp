...
<body>
	<%-- <div class="mini-toolbar">
		<a class="mini-button"   onClick="addJob">新增</a>
	...
	<div class="mini-toolbar">
         <ul class="toolBtnBox">
			...
			<li>
				<a id="btnStart"  class="mini-button" onClick="switchScheduler('true')" style="display:${!isStarted?'blocked':'none'}">启动定时器</a>
			</li>
			 <li>
				 <a id="btnStop" class="mini-button"  onClick="switchScheduler('false')" style="display:${isStarted?'blocked':'none'}" >停止定时器</a>
			 </li>
		</ul>
     </div>
	<div class="mini-fit rx-grid-fit" style="height: 100%;">
		...
		>
			<div property="columns">
				...
				<div field="name" width="120"  allowSort="true">任务名称</div>
				<div field="className" width="120"  allowSort="true">类名</div>
				<div field="description" width="120"  allowSort="true">备注</div>
			</div>
		</div>
	</div>
	...
	function jobEdit(name) {
		var url=encodeURI(__rootPath + "/sys/core/scheduler/jobEdit.do?name=" +name);
		_OpenWindow({
			url : url ,
			title : "编辑任务",
			width : 820,
	...
	function switchScheduler(status) {
  	 ...
  	 function onRenderer(e) {
		...
		function onActionRenderer(e) {
			...
			var s = '<span  title="编辑" onclick="jobEdit(\''+ name+ '\')">编辑</span>'
					+ ' <span  title="删除" onclick="delJob(\''+ name + '\')">删除</span>'
					+ ' <span  title="执行任务" onclick="runJob(\'' + name + '\')">执行任务</span>'
					+ ' <span  title="计划列表" onclick="gotoPlan(\'' + name + '\')" >计划列表</span>'
					+ ' <span  title="日志列表" onclick="getLog(\'' + name + '\')" >日志列表</span>';
			return s;
		}
