...
<body>
	...
	<div class="form-container">
		<form id="form1" method="post">
			...
			<input id="table" name="echartsType" class="mini-hidden" value="common" />
			...
			<div id="tabs1" class="mini-tabs" activeIndex="0" plain="false" buttons="#tabsButtons">
				...
				<div title="数据字段" showCloseButton="false">
					...
					<table class="table-detail column-six" cellspacing="1" cellpadding="0" style="margin-top: 0;">
						<tr>
							<td>开启双Y轴</td>
							<td><input id="openDoubleYAxis" class="mini-radiobuttonlist" valueField="id" textField="text"
									data="[{'id':'0', 'text':'否'}, {'id':'1', 'text':'是'}]" value="0"/></td>
							<td>是否堆叠</td>
							<td><input id="openStack" class="mini-radiobuttonlist" valueField="id" textField="text"
									data="[{'id':'0', 'text':'否'}, {'id':'1', 'text':'是'}]" value="0"/></td>
							<td>XY轴转换</td>
							<td><input id="xyConvert" name="xyConvert" class="mini-radiobuttonlist" valueField="id" textField="text"
									data="[{'id':0, 'text':'否'}, {'id':1, 'text':'是'}]"
									value="${not empty echartsCustom.xyConvert && echartsCustom.xyConvert == 1 ? 1 : 0}"/></td>
						</tr>
						<tr>
							<td>折线平滑</td>
							<td><input id="openLineSmooth" class="mini-radiobuttonlist" valueField="id" textField="text"
									data="[{'id':'0', 'text':'否'}, {'id':'1', 'text':'是'}]" value="0"/></td>
							<td>缩放区域</td>
							<td colspan="3"><input id="dataZoom" name="dataZoom" class="mini-radiobuttonlist" valueField="id" textField="text"
									data="[{'id':'0', 'text':'关闭'}, {'id':'1', 'text':'开启'}]" value="${empty echartsCustom.dataZoom || echartsCustom.dataZoom=='0' ? 0 : 1}"/></td>
						</tr>
						<tr>
							<td>图形边距</td>
							<td colspan="5">
					...
					<div id="gridQuery" class="mini-datagrid" style="width: 100%;" showPager="false" allowCellEdit="true" allowCellSelect="true"
						...
						<div property="columns">
							...
							<div field="shapeType" width="60" headerAlign="center" type="comboboxcolumn">
								图形形状 <input class="mini-combobox" property="editor" valueField="id" textField="type"
									data="[{'id':'bar', 'type':'柱状'}, {'id':'line', 'type':'线性'}]" />
							</div>
							<div field="smooth" width="60" headerAlign="center" visible="false" type="comboboxcolumn">
								折线平滑<input class="mini-combobox" property="editor" valueField="id" textField="type"
									data="[{'id':'0', 'type':'否'}, {'id':'1', 'type':'是'}]" />
							</div>
							<div field="yAxisIndex" width="60" headerAlign="center" visible="false" type="comboboxcolumn">
								Y轴<input class="mini-combobox" property="editor" textField="value" valueField="id"
									data="[{'id':'0', 'value':'第一Y轴'}, {'id':'1', 'value':'第二Y轴'}]" />
							</div>
							<div field="stack" width="60" headerAlign="center" visible="false" type="comboboxcolumn">
								堆叠<input class="mini-combobox" property="editor" textField="value" valueField="id"
									data="[{'id':'0', 'value':'否'}, {'id':'1', 'value':'是'}]" />
							</div>
						</div>
					</div>
				</div>
				<div title="x轴栏位" showCloseButton="false">
					<div class="form-toolBox">
					...
					<div class="form-toolBox">X轴指示器设置</div>

					<table class="table-detail column-two" cellspacing="1" cellpadding="0" style="margin-top: 0;">
						<tr>
							<td>是否显示</td>
							<td><input id="xAxis-axisPointer-show" class="mini-radiobuttonlist"
								valueField="id" textField="text"
								data="[{'id':'0', 'text':'是'},{'id':'1', 'text':'否'}]" value="0"/></td>
						</tr>
						<tr>
							<td>指示类型</td>
							<td><input id="xAxis-axisPointer-type" class="mini-radiobuttonlist"
								valueField="id" textField="text"
								data="[{'id':'line', 'text':'直线'},{'id':'shadow', 'text':'阴影'}]" value="line"/></td>
						</tr>
					</table>
				</div>
	...
	<script type="text/javascript">
		...
		//x轴坐标属性设置
		function setXAxisProperty(){
			var xAxis = {};
			var axisPointer = {};
			axisPointer.show = (mini.get("xAxis-axisPointer-show").getValue() == "0") ? true : false;
			axisPointer.type = mini.get("xAxis-axisPointer-type").getValue();
			xAxis.axisPointer = axisPointer;
			var _xAxisField = {
				name : "xAxisField",
				value: xAxis
			}
		...
		function selfSaveData() {
			...
			for (var i = 0; i < dataGridQuery.length; i++) {
				...
				obj.shapeType = (dataGridQuery[i].shapeType == undefined ? "bar" : dataGridQuery[i].shapeType); //添加当前数据使用什么形状显示, version-1: bar, line
				if(mini.get("openDoubleYAxis").getValue() == 1){
					obj.yAxisIndex = (dataGridQuery[i].yAxisIndex == undefined ? 0 : dataGridQuery[i].yAxisIndex);
				}
				if(mini.get("openStack").getValue() == 1){
					obj.stack = (dataGridQuery[i].stack == undefined ? 0 : dataGridQuery[i].stack);
				}
				if(mini.get("openLineSmooth").getValue() == 1){
					obj.smooth = (dataGridQuery[i].smooth == undefined ? 0 : dataGridQuery[i].smooth);
				}
				queryData.push(obj);
			}
			...
			formData.push(setXAxisProperty());
