    ...
    <div class="form-container">
		...
		<div class="mini-toolbar">
			...
			<form id="form1" class="search-form">
						<ul class="search-ui">
									...
									<c:when test="${column.valueSource=='param'}">
										<c:choose>
											<c:when test="${column.typeOperate=='BETWEEN'}">
												<span class="text">${column.comment}：</span><input name="${column.fieldName}_start" class="mini-textbox" /> 到<input name="${column.fieldName}_end" class="mini-textbox" />
											</c:when>
											
											<c:otherwise>
												<span class="text">${column.comment}：</span><input name="${column.fieldName}" class="mini-textbox" />
											</c:otherwise>
										</c:choose>
									</c:when>
								</c:choose>
							...
							<li class="liBtn">
								<a class="mini-button"   plain="true" onclick="doCustomQuery()">查询</a>
								<a class="mini-button"	  plain="true" onclick="clearQuery()">清空查询</a>
							</li>
						</ul>
				</form>
			</div>
		...
		<div class="mini-fit">
			...
			<div style="width:100%;height:300px;">
				<textarea class="mini-textarea" id="sqlDiy" name="sqlDiy" emptyText="请输入"  style="width:100%;height:100%;"></textarea>
			</div>
		</div>
