...
<body>
	...
	<script type="text/javascript">
		...
		function onActionRenderer(e) {
			...
			var s = '<span  title="节点属性配置" onclick="nodeProperties(\'' + uid + '\')">节点属性配置</span>';
			if(node.type=='process' || node.type=='userTask'){
				s +=  '<span title="催办配置" onclick="reminder(\'' + uid + '\')">催办配置</span>';
			}
			if (node.type == 'process'){
				s += '<span  title="抄送设置" onclick="copyto(\'' + uid + '\')">抄送设置</span>';
			}
		...
		function nodeProperties(uid) {
			...
			_OpenWindow({
				...
				width : 600,
				height : 500,
				url : __rootPath + '/bpm/core/bpmNodeSet/getNodeConfig.do?actDefId=${bpmDef.actDefId}&nodeId=' + node.activityId + '&nodeName='+node.name+'&nodeType=' + node.type + '&solId=' + solId
			});
