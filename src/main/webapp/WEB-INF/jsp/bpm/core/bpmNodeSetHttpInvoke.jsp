			...
			<c:if test="${fn:length(configVars)>0}">
				...
				<table class="table-detail column_2_m" cellspacing="1" cellpadding="0">
					...
					<tr>
						...
						<td >
							<input id="set" class="mini-buttonedit icon-dep-button"  required="true" allowInput="false" onbuttonclick="selectDataBind"/>
							<a class="mini-button" iconCls="icon-next" onclick="loadApp">测试</a>
						</td>
						<th >
							<span class="starBox">
								是否异步<span class="star">*</span>
							</span> 
						</th>
						<td >
							  <input id="async" class="mini-checkbox" truevalue="YES" falsevalue="NO" />
						</td>
					</tr>
					<tr>
						<th>
						...
						<td colspan="3">
						<div id="tabs" class="mini-tabs" activeIndex="0" plain="false"
						buttons="#tabsButtons" onactivechanged="activeChanged">
							<div title="参数设置">
						...
						<td colspan="3">
								<div class="mini-toolbar" style="margin-bottom:2px;padding:10px;">
							 		...
							 		<div class="div-helper" >
						    			<div  class="iconfont helper icon-Help-configure" title="帮助" placement="e" helpid="scriptHelp"></div>
						    		</div>
								</div>
...
<pre>
上下文变量:
bpmParam:流程变量为一个MAP对象。
formParam:表单数据对象为一个MAP对象。
actInstId:流程实例ID。
cmd:cmd对象。
...
result: WEB调用后返回的结果数据。
	...
	<script type="text/javascript">
		...
		function init(){
			$.ajax({
	             ...
	             success: function(data){
				 	...
				 	var aa = mini.get("set");
				 	aa.setText(text);
				 	$("#key").text(key==null?'':key);
				 	$("#url").text(url==null?'':url);
		...
		function saveConfig(){
			...
			var async = mini.get("async").getValue();
			var configJson={type:"HttpInvoke",paramsData:paramsData,key:$("#key").text(),script:script,async:async};
			_SubmitJson({
