	...
	winHeight="450" winWidth="700" entityTitle="流程实例"
		...
		$(function(){
	        ...
	        function onActionRenderer(e) {
	            ...
	            var s = '<span class="" title="明细" onclick="myDetailRow(\'' + pk + '\')">明细</span>';
	            
	            if(record.status=='RUNNING'){
	            	s+= '<span class="" title="催办" onclick="doUrge(\'' + pk + '\')">催办</span>';
	            }
	            ...
	            for(var i=0;i<instJson.length;i++){
           			...
           			console.log(getByJson(record,json)+"<br/>")
           		}
        	 ...
        	 function getByJson(record,json){
        		...
        		switch(json.key){
        			case "del":
        				if(json.val){
        					str='  <span class="" title="删除" onclick="delRow(\'' + pkId + '\')">删除</span>';
        				}
        				break;
        			case "intervene":
	        ...
	        function doUrge(pkId){
        		var obj=getWindowSize();
                _OpenWindow({
                	url: __rootPath+"/bpm/core/bpmInst/urge.do?instId=" + pkId,
                    title: "流程催办", width: 600, height: 400
                });
        	}
        	...
        	function myDetailRow(pkId){
                ...
                _OpenWindow({
                    ...
                    title: "流程明细", width: 1000, height: 600,max:true
                });
        	}
