...
<script type="text/javascript">
	...
	function OnCellBeginEdit(e) {
        ...
        if (field == "edit" || field == "read" || field == "require") {
	...
	function saveRights(){
		...
		if(tables.length>0){
			...
			tables.each(function(){
				...
				var relationType=table.attr("relationType");
				
				var obj={name:tableName,comment:comment,relationType:relationType};
	...
	function batSetRight(field,gridId){
		...
		var pop =$("#settingPanel").closest(".mini-popup");
		if(pop.css("left")=="0px"){
			var screenWidth = $(window).width();
			var screenHeight = $(window).height();
			var scrolltop = $(document).scrollTop();
			var scrollleft = $(document).scrollLeft();
			var objLeft = (screenWidth - pop.width())/2 + scrollleft  ;
			var objTop = (screenHeight - pop.height())/2 + scrolltop;
			pop.css({left: objLeft + 'px', top: objTop + 'px'});
		}
	}
...
<script type="text/html;" id="rightTemplate">
			...
			<#
				for(var i = 0; i<sub.length;i++){
					var obj=sub[i];
					var relationType=obj.relationType;
					
			#>
			<table id="table_<#=obj.name#>" class="subTable" name="<#=obj.name#>" comment="<#=obj.comment#>" relationType="<#=relationType#>">
				<tr>
					<th><#=obj.comment#>[<#=obj.name#>]</th>
				</tr>
				<tr>
					<td style="background-color:white;">
						...
						<li>
						<span>添加：</span><input name="add" textName="addName" class="mini-popupedit"  textField="names" 
							valueField="ids" popupWidth="auto" showClose="true"
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
						<li>				
						<span>编辑：</span><input name="edit" textName="editName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
						<li>						
						<span>删除：</span><input name="del" textName="delName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
						<#}#>
						<li>
						<span>隐藏:</span><input name="hidden" textName="hiddenName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						...
						<#if(relationType=="onetomany"){#>
						<li>
						<span>必填:</span><input name="require"  textName="requireName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true"  onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
						<li>
						<span>编辑已添加:</span><input name="editExist" textName="editExistName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
						<li>
						<span>删除已添加:</span><input name="delExist" textName="delExistName" class="mini-popupedit"  textField="names" showClose="true"
							valueField="ids" popupWidth="auto" 
							showPopupOnClick="true" onshowpopup="showPopup" onhidepopup="hidePopup"
                			popup="#settingPanel"  multiSelect="true" allowInput="false" oncloseclick="_ClearButtonEdit" />
						</li>
				...
				<tr>
					<td>
						 <a class="mini-button"  plain="true" onclick="batSetRight('edit','grid_<#=obj.name#>')">编辑</a>
   	 					 <a class="mini-button" plain="true" onclick="batSetRight('read','grid_<#=obj.name#>')">只读</a>
   	    				 <a class="mini-button" plain="true" onclick="batSetRight('require','grid_<#=obj.name#>')">必填</a>
					</td>
				</tr>
			</table>
			...
			意见权限:
			<div>
			 	<a class="mini-button"  plain="true" onclick="batSetRight('edit','gridOpinion')">编辑</a>
   	 			<a class="mini-button" plain="true" onclick="batSetRight('read','gridOpinion')">只读</a>
   	  			<a class="mini-button" plain="true" onclick="batSetRight('require','gridOpinion')">必填</a>
			</div>
			<div id="gridOpinion" 
	         	class="mini-datagrid" 
	         	style="width:100%;"     
			...
			按钮权限:
			<div>
			 	<a class="mini-button"  plain="true" onclick="batSetRight('visible','gridButton')">显示</a>
...
<body>
	<div class="topToolBar">
		<div>
		   	 <a class="mini-button"  plain="true" onclick="saveRights">保存</a>
		   	 <a class="mini-button btn-red"  plain="true" onclick="clearRights">重设</a>
		   	 <a class="mini-button"  plain="true" onclick="batSetRight('edit','mainGrid')">编辑</a>
		   	 <a class="mini-button"  plain="true" onclick="batSetRight('read','mainGrid')">只读</a>
		   	 <a class="mini-button"  plain="true" onclick="batSetRight('require','mainGrid')">必填</a>
		</div>
	</div>
	<div class="mini-fit">
		<div class="form-container">
			...
			<div style="display:none">
					<input id="rightEditor" class="mini-popupedit"  textField="names" valueField="edit" popupWidth="auto"
						showPopupOnClick="true" showClose="true" popupHeight="390"
						popup="#settingPanel"  multiSelect="true" allowInput="false" onshowpopup="showPopup" onhidepopup="hidePopup" oncloseclick="_ClearButtonEdit"/>
			</div>

			 <div id="settingPanel" class="mini-panel" title="设置权限"  style="width:550px;height:100%;overflow: hidden" visible="false"
					showToolbar="true" showCloseButton="true" showHeader="true"  >

					<div>
						<div class="form-toolBox">
							<ul>
								<li><a class="mini-button"  onclick="onSettingSave()">保存</a> </li>
								<li><a class="mini-button btn-red"  onclick="onSettingClose()">关闭</a></li>
							</ul>
						</div>
						<form id="viewForm">
							<table class="table-detail column-four" cellpadding="0" cellspacing="1" style="width:100%">
								...
								<tr id="groupRow" class="row">
									<td>用户组</td>
									<td>
										<input id="groupLinks" name="groupLinks" allowInput="false" class="mini-textboxlist"  style="width:200px;height: 30px" value="" text="" />
										<a class="mini-button"  plain="true" onclick="addGroups">用户组</a>
								...
								<tr id="subGroupRow" class="row">
									<td>用户组(包括下级)</td>
									<td>
										<input id="subGroupLinks" name="subGroupLinks" allowInput="false" class="mini-textboxlist"  style="width:200px;height: 30px" value="" text="" />
										<a class="mini-button"  plain="true" onclick="addSubGroups">用户组</a>
