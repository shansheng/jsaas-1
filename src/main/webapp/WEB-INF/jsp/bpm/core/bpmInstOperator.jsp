...
<body>
			<div class="topToolBar">
				<div>
					...
					<a class="mini-button"  onclick="showInstDetail()">实例明细</a>
					<a class="mini-button"  onclick="modifyInstVars()">修改流程变量</a>
					<a class="mini-button"  onclick="modifyFormInst()">修改表单数据</a>
					<a class="mini-button"  onclick="showProcessProperties()">属性配置</a>
					<a class="mini-button"  onclick="bpmSolutionPros()">方案配置</a>
					<a class="mini-button"  onclick="location.reload()">刷新</a>
			...
			<div class="form-container" >
					...
					<ul id="jumpMenu" class="mini-menu" style="display:none;">
						<c:forEach items="${taskNodeUsers}" var="taskNode">
							<c:if test="${taskNode.running!=true}">
							  <li  onclick="changeFlowPath('${taskNode.nodeId}','${taskNode.nodeText}')">跳至【${taskNode.nodeText}】</li>
							</c:if>
						</c:forEach>
					</ul>
				    ...
				    <div class="nodeBO">
				    	<table class="table-view" cellpadding="0" cellspacing="1" style="width:100%">
					    	<tr>
					    		<td width="15%">节点名称</td>
					    		<td width="25%">流程人员配置</td>
					    		<td width="35%">干预人员配置</td>
					    		<td width="25%">审批人员</td>
					    	</tr>
							<c:forEach items="${taskNodeUsers}" var="taskNode" >
								<tr class="table-row"<c:if test="${taskNode.running==true}"></c:if>>
									...
									<td>
										...
										<a class="mini-button"  plain="true" onclick="changeNodeUserSet('${taskNode.nodeId}','${taskNode.nodeText}')">设置</a>
									</td>
									<td>
										...
										<a class="mini-button"  plain="true" onclick="onSetting('${taskNode.nodeId}')">设置</a>
										<a class="mini-button" " plain="true" onclick="onClear('${taskNode.nodeId}')">清空</a>
										
										<a class="mini-button"   plain="true" onclick="copyRef('${taskNode.nodeId}')">复制</a>
												
									</td>
									<td>
										${taskNode.exeUserFullnames}
										<c:if test="${taskNode.running==true}">
											<a class="mini-button"  plain="true" onclick="changeTaskUser('${taskNode.nodeId}','${taskNode.exeUserIds }','${taskNode.exeUserFullnames}')">更改执行人</a>
											<a class="mini-menubutton" menu="#jumpMenu"    onclick="taskNodeClick('${taskNode.nodeId}','${taskNode.nodeText}')">更改执行路径</a>
										</c:if>
									</td>
								</tr>
							</c:forEach>
		    ...
		    <form id="opinionForm">
		    	<div id="toolbar1" class="mini-toolbar mini-toolbar-bottom" >
				    <table width="100%">
				        <tr>
				      		<td style="width:100%;">
								<a class="mini-button"   plain="true" onclick="changePath()">确定</a>
		   						<a class="mini-button btn-red"    plain="true" onclick="closePathWin()">关闭</a>
							</td>
						</tr>
					</table>
				</div>
