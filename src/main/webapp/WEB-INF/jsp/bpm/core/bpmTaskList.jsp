				...
				<div class="mini-fit">
					<div 
						...
						<div property="columns">
							...
							<div field="assigneeNames"  width="50" headerAlign="" >执行人</div>
							<div field="stayTime" width="50" headerAlign="" >停留时间</div>
							<!-- div field="agentUserId"  sortField="agent_user_id_"width="50" headerAlign="center" allowSort="true">代理人</div-->
							<div field="createTime" sortField="create_time_" width="80" dateFormat="yyyy-MM-dd HH:mm:ss" headerAlign="" allowSort="true">创建时间</div>
							<div field="dueDate" sortField="due_date_" width="60" headerAlign="" allowSort="true">到期时间</div>
							<div field="overtime"  sortField="overtime_" width="40" headerAlign="" allowSort="true" renderer="onOvertimeRenderer">超时信息</div>
						</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
       	...
       	function getByJson(record,json){
            ...
            var name=record.name;
       		...
       		switch(json.key){
       			case "intervene":
       				if(json.val){
       					str=' <span class="" title="干预" onclick="opRow(\'' + uid + '\')">干预</span>';
       				}
       				break;
       			case "handle":
       				if(json.val){
       					str='<span class="" title="办理" onclick="checkAndHandTask(\'' + uid + '\',true)">办理</span>';
       				}
       				break;
       		}
       		return str;
        ...
        grid.on("drawcell", function (e) {
            ...
            if(field=='description'){
            	if(value){
            		var str= '<a href="javascript:checkAndHandTask(\'' + record._uid + '\',true)">';
            		if(record.taskType=='CMM'){
            			str+='<img src="'+__rootPath+'/styles/icons/commute.png"> ';
            		}
            		e.cellHtml=str+record.description+'</a>';
            	}else{
            		e.cellHtml='<a href="javascript:checkAndHandTask(\'' + record._uid + '\',true)">'+record.name+'</a>';
            	}
            }
            if(field=='dueDate'){
            	if(value){
