...
<body>
 ...
 <div class="mini-fit rx-grid-fit" style="padding-top:12px;">
            <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;border" allowResize="false"
                 ondrawnode="onDrawNode" url="${ctxPath}/flie/tray/flieTrayBasicJpa/listData.do" onnodedblclick="openNew()" idField="ID_" multiSelect="true" showColumnsMenu="true"
			...
			<div property="columns">
				...
				<div  field=""  width="60" headerAlign="center" allowSort="true">权限</div>
			</div>
		</div>
	</div>
...
// 	mini.parse();	
	...
	{
			_OpenWindow({
				url : '${ctxPath}/flie/tray/flieTrayBasic/edit.do', 
				title : "新建文件夹",
				width : 600,
				height : 400,
				method : 'POST',
	...
	//删除节点
	  ...
	  */
			...
			if(permissionValue=="admin")
			{
		    	 ...
		    	 s+='<span class="icon-form" title="编辑权限" onclick="PermissionRow(\'edit\',\'' + pkId + '\')"></span>';
			...
			$.ajax({
				type : "POST",
				url : '${ctxPath}/online/tree/inquirePermissions.do',
				data : {folderId:folderId},
				async: false,
				success : function(result) {
					if(!result.onlineAddP)
					$("#newFile").css("display","none");
					if(result.onlineEditP)	//没有值 如果是管理员 就会出现控制权限与删除 否则子目录只有编辑且没有保存
					{
						 s+= ' <span class="icon-edit" title="编辑" onclick="editMyRow()"></span>'; 
					}
				    if(result.onlineDeleteP)	
				    	 s+= ' <span class="icon-remove" title="删除" onclick="delMyRow()"></span>';
				    	 s+='<span class="icon-form" title="编辑权限" onclick="PermissionRow(\'edit\',\'' + pkId + '\')"></span>';
				    	 s+='<span class="icon-grant" title="阅读权限" onclick="PermissionRow(\'read\',\'' + pkId + '\')"></span>';
				}
