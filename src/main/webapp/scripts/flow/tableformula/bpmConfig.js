var fieldEditors=[{mapType:'',mapTypeName:'',editor:null},
		...
		{mapType:'subField',mapTypeName:'从子表字段获取',editor:'subFieldEditor',hasValue:true},
...
function loadData(data,el){
	...
	var subTable=getSubTable(el);
	subTable.setData(data.results);
	if(subTable.defaultValue){
		var filterSql = mini.getByName("filterSql_"+subTable.id.split("_")[1]);
		for(var i=0;i<subTable.data.length;i++){
			var subData = subTable.data[i];
			if(subData.id==subTable.getValue()){
				filterSql.boDefId=subData.boDefId;
				break;
			}
		}
		filterSql.entName=subTable.getText();
