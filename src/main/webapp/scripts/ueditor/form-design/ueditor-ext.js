...
function parseGrid(){
	...
	for(var i=0;i<grids.length;i++){
	   var grid=grids[i];
	   if( $(grid).hasClass("relation-grid")) continue;
	   var id=grid.id;
	   if(!id) continue;
	   var gd=mini.get(id);
	   if(!gd)  continue;
...
function handOpinion(){
	var grid=mini.get("checkhilist");
	if(grid){
		grid.load();
 ...
 * @returns
 */
function parseWordBtn(){
	$(".form-model").each(function(){
		var obj=mini.getByName("ID_",this);
		if(!obj) {
			mini.findControls(function(control){
			   if(control.type=="wordcontrol") {
