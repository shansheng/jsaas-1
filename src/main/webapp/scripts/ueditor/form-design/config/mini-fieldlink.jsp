...
<body>
	...
	<div id="toolbar1" class="mini-toolbar mini-toolbar-bottom" >
	    <table width="100%">
	        <tr>
	      		<td style="width:100%;">
					<a class="mini-button"  plain="true" onclick="toggleAdvSearch">高级自定义查询绑定</a>
				</td>
			</tr>
		</table>
	</div>
			...
			<form id="configForm">
				<table class="table-detail column-four" cellspacing="0" cellpadding="1">
					...
					<tr>
						<td>URL快捷方式</td>
						<td colspan="3">
							<a class="mini-button"  menu="" onclick="selCustomQueryForm">获取自定义表单URL</a>
							&nbsp;
							替换参数
							<input class="mini-combobox" name="paramFields" id="paramFields" valueField="name" textField="comment" onvaluechanged="changeParamField"/>
								<ul id="popVarsMenu" style="display:none" class="mini-menu">
							 		<li   onclick="appUrlParam('ctxPath')">应用程序路径</li>
							 		<li   onclick="appUrlParam('curUserId')">当前用户ID</li>
							 		<li   onclick="appUrlParam('curDepId')">当前主部门</li>
							    </ul>
							<a class="mini-menubutton "  menu="#popVarsMenu" >插入变量</a>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>连接旁边图标</td>
						<td>
							<input name="iconCls"   class="mini-buttonedit" onbuttonclick="selectIcon" style="width:70%"/>
							<a class="mini-button" id="icnClsBtn"></a>
						</td>
					</tr>
				</table>
				<table class="table-detail column-four" style="display:none" id="advQueryTable">
					<tr>
					...
					<tr>
						...
						<td colspan="3">
							<input class="mini-buttonedit" id="cusQuery" name="cusQuery" showClose="true" oncloseclick="_ClearButtonEdit" onbuttonclick="selectCustomQuery" style="width:250px;"/>
						</td>
					</tr>
					<tr>
						<td>绑定参数</td>
