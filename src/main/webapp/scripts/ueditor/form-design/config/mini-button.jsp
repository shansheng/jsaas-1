...
<style type="text/css">
body  .MyBlock .mini-button-icon{
	display:block;
}
body .MyBlock .mini-button-text{
	padding:4px 10px;
}
body .MyBlock .mini-button-icon{
	margin-top:-12px;
...
.MyBlock .mini-button-icon-only .mini-button-icon{
	left:3px;
}
</style>
</head>
<body>
			...
			<form id="miniForm">
				<table class="table-detail column-four" cellspacing="0" cellpadding="1">
					...
					<%--	<td>按钮样式</td>
						...
						<td colspan="3"><input class="mini-checkbox" name="enabled" id="enabled" trueValue=true falseValue="false" checked="true" />是
						</td>
					...
					<tr id="checkdlgTr">
						<td>自定义对话框</td>
						<td colspan="3">	
    						<div id="ckselfdlg" name="ckselfdlg" class="mini-checkbox" value="false" readOnly="false" text="是" onvaluechanged="onSelfDlgChanged" style="font-size: 14px"></div>
						...
						<td colspan="3">
							<div class="mini-toolbar" >
							    <table style="width:100%;">
							        <tr>
								        <td style="width:100%;">
								            <a class="mini-button btn-red" onclick="removeRow">删除</a>
								            <input id="table" name="table" class="mini-combobox" textField="comment" valueField="name" onvaluechanged="tableChange" />
								        </td>
							        </tr>
							    </table>
							</div>
	...
	<script type="text/javascript">
		...
		dialog.onok = function (){
		    ...
		    if( !oNode ) {
		        try {
					...
					oNode.setAttribute('style','color:#fff');
	   ...
	   function initUniqueField(){
		   ...
		   if(!table.getValue() || table.getValue()=="main") {
			   spanObj.hide();
			   var tableValueList =[];
			   for(var i in metaJson){
				   var comment =metaJson[i].comment;
				   if(comment){
					   var tableValue ={
						   "comment":comment,
						   "name":i
					   }
					   tableValueList.push(tableValue);
				   }
			   }
			   table.setData(tableValueList);
		   }else{
			   var val=uniqueField.getValue();
			   var tableName=table.getValue();
