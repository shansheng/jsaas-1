...
function FormCalc(formId){
	...
	*/
	this.comboBoxSubTable=function(e){
         ...
         if(!comboboxSetting[field]) return ;
                  
    	 var parent = comboboxSetting[field];
         var val = "";
         if(sql.mainfield=="no"){
        	 val=record[parent];
         }
         else{
        	 var obj=mini.getByName(parent);
        	 val=obj.getValue();
         }
	...
	this.calcDateSubTableStart=function(e){
		...
		for(var key in dateStart){
			...
			if(setting.type=="today"){
				...
				else if(setting.compare=="LTE"){
					ctl.setMaxDate(new Date().Max());
				}
				else if(setting.compare=="LT"){
					ctl.setMaxDate(new Date());
				}
			}
			...
			control.on("focus",function(e){
				...
				if(setting.type=="today"){
					if(setting.compare=="GTE"){
						...
						ctl.setMaxDate(new Date().Min().sub("day",1));
					}
					else if(setting.compare=="LT"){
						ctl.setMaxDate(new Date());
					}
	...
	this.parseMainDate=function(){
		var controls=mini.findControls(function(control){
			if(control.type=="textbox" || control.type=="spinner" || control.type=="datepicker") return true;
			return false;
		});
		for(var i=0;i<controls.length;i++){
			var ctl=controls[i];
			...
			var ary = this.handleControls(setting);
			for(var j=0;j<ary.length;j++){
				this.bindDateChange(ary[j],ctl,setting);
			}
		}
	 ...
	 * 主表日期数据变化。
	 */
	this.bindDateChange=function(ctl,targetObj,setting){
		var self=this;
		ctl.on("valuechanged",function(e){
			...
			var set = self.handleSetting(data,setting);
			targetObj.setValue(eval(set));
			targetObj.doValueChanged();
		})
	},
	
	this.handleControls=function(setting){
		var reg=/\{(\w*?)\}/g;
		setting = setting.match(reg);
		var controls = [];
		for(var i=0;i<setting.length;i++){
			var match = setting[i];
			var name = match.substring(1,match.length-1);
			controls.push(mini.getByName(name));
		}
		return controls;
	},
	
	this.handleSetting=function(data,setting){
		var reg=/\{(\w*?)\}/g;
		setting = setting.replace(reg,function(match){
			var obj = data[match.substring(1,match.length-1)];
	...
	this.parseMain=function(){
		...
		for(var i=0;i<controls.length;i++){
			...
			if(regStr.test(formula)){
				//传入ctl 控件。
				this.parseGridGlobal(ctl);
			}
			//主表字段变化
			this.parseMainField(ctl);
		}
	},
	 ...
	 * {(数量)amount}*{total}
	 */
	this.parseMainField=function(ctl){
		//formula,name,decimal
		var formula=ctl.formula;
		var name=ctl.name;
		//解析公式关联的主表字段。
		...
		this.globalFormula[name]={fields:fields,ctl:ctl};
	},
	 ...
	 * 
	 */
	this.parseGridGlobal=function(ctl){
		var formula=ctl.formula;
		var name=ctl.name;
		...
		while(matchSub){
			...
			gridObj[tableName]={};
			gridObj[tableName][name]={fields:fields,ctl:ctl};
	...
	this.parseGrid=function(){
		...
		for(var i=0;i<grids.length;i++){
			...
			for(var j=0;j<cols.length;j++){
				...
				formulaObj[gridName][col.name]={fields:fields,col:col};
			}
			//{子表1:{field1:{fields:[],col:col}}}
	 ...
	 * 判断字段是否在公式中存在。
	 * 
	 * formulas:{field1:{fields:[],col:col}}
	 */
	this.isFieldExist=function(field,formulas){
		...
		for(var key in formulas){
			...
			if(formulaObj.col.formula.indexOf(field)!=-1){
				rtn=true;
				break;
			}
		}
	 ...
	 * 参数说明:
	 * data:一行数据
	 * obj:{col:col,fields:[field1,field2]}
	 */
	this.calcValue=function(data,obj){
		var col=obj.col;
		var formula=col.formula;
		var decimal=col.decimal || 2;
		...
		val=MathUtil.roundFixed(val,decimal);
		return val;
	},
	...
	this.calcMainValue=function(data,obj){
		var formula=obj.ctl.formula;
		var decimal=obj.ctl.decimal || 2;
		var fields=obj.fields;
		this.setFieldWhenEmpty(data,fields);
		var regStr = /\{(\(.*?\)){0,1}(\w*?)\}/g;
	...
	this.handRowFormula=function(grid,formulas,val,field,row){
		...
		formData[grid.id.replace("grid_","")]=row;
		
		for(var key in formulas){
			var formulaObj=formulas[key];
			var val=this.calcValue(formData,formulaObj);
			var obj={};
			obj[key]=val;
			grid.updateRow(row,obj);
		}
	 ...
	 * 
	 */
	this.handGridData=function(targetField){
		
		var targetObj=mini.getbyName(targetField);
		var decimal=targetObj.decimal || 2;
		var formula=targetObj.formula;
	...
	this.parseMainCascadeQuery=function(){
		var controls=mini.findControls(function(control){
			if(control.type=="combobox" && control.sql) {
				var el=$(control.el);
				//只取子表的combox控件。
				if(el.closest("div.rx-grid").length==0) return true;
			}
			return false;
		});
		for(var i=0;i<controls.length;i++){
