...
function onHeaderCellClick(e){
	var sortField = e.column.field;
	var data = e.sender.data;
	var ary = bubbleSort(data,sortField);
	e.sender.setData(ary);
}
...
function bubbleSort(arr,sortField) {
    ...
    for (var i = 0; i < len; i++) {
        for (var j = 0; j < len - 1 - i; j++) {
            if ((hashCode(arr[j][sortField]) > hashCode(arr[j+1][sortField]) && flag) || (hashCode(arr[j][sortField]) < hashCode(arr[j+1][sortField]) && !flag)) {        //相邻元素两两对比
